...
EOS
      ...
      cmd_write=(
        time_json -o "${JOB_OUTPUT_DIR}/${runid}_w.json"
        "${cmd_mpirun[@]}"
        -mca hook_pmembb_load false
        -mca hook_pmembb_save true
        "${cmd_ior[@]}"
        # -O "stoneWallingStatusFile=${JOB_OUTPUT_DIR}/ior_stonewall_w_${runid}"
        -O "summaryFile=${JOB_OUTPUT_DIR}/ior_summary_w_${runid}.json"
        -w
      )

      cmd_read_remote=(
        time_json -o "${JOB_OUTPUT_DIR}/${runid}_r.json"
        "${cmd_mpirun[@]}"
        -mca hook_pmembb_load true
        -mca hook_pmembb_save false
        "${cmd_ior[@]}"
        # -O "stoneWallingStatusFile=${JOB_OUTPUT_DIR}/ior_stonewall_r_${runid}"
        -O "summaryFile=${JOB_OUTPUT_DIR}/ior_summary_r_${runid}.json"
        -r
        -C
        -Q 1
      )
      ...
      cmd_read_local=(
        time_json -o "${JOB_OUTPUT_DIR}/${runid}_r_l.json"
        "${cmd_mpirun[@]}"
        -mca hook_pmembb_load true
        -mca hook_pmembb_save false
        "${cmd_ior[@]}"
        # -O "stoneWallingStatusFile=${JOB_OUTPUT_DIR}/ior_stonewall_r_l_${runid}"
        -O "summaryFile=${JOB_OUTPUT_DIR}/ior_summary_r_l_${runid}.json"
        -r
      )
      ...
      echo "${cmd_write[@]}"
      "${cmd_write[@]}" \
        > >(tee "${JOB_OUTPUT_DIR}/ior_w_stdout_${runid}.txt") \
        2> >(tee "${JOB_OUTPUT_DIR}/ior_w_stderr_${runid}.txt" >&2)
      ...
      "${cmd_read_remote[@]}" \
        > >(tee "${JOB_OUTPUT_DIR}/ior_r_stdout_${runid}.txt") \
        2> >(tee "${JOB_OUTPUT_DIR}/ior_r_stderr_${runid}.txt" >&2)
      ...
      echo "${cmd_read_local[@]}"
      "${cmd_read_local[@]}" \
        > >(tee "${JOB_OUTPUT_DIR}/ior_r_l_stdout_${runid}.txt") \
        2> >(tee "${JOB_OUTPUT_DIR}/ior_r_l_stderr_${runid}.txt" >&2)
